...
def retirement_403b_calculator(
    ...
    for age in range(current_age+1,retirement_age+1):
        yearly_data=dict()
        yearly_data["age"]=age
        yearly_data["current_salary"]=round(current_salary*(1+annual_salary_growth_rate)**(age-current_age),2)
        yearly_data["employee_contribution"]=round(min(30500 if (current_age>=50 and max_out) else 23000,yearly_data["current_salary"]*annual_contribution_percentage),2)
        yearly_data["employer_contribution"]=round(min(53000,employer_match_percentage*yearly_data["current_salary"]),2)
        yearly_data["total_contribution"]=round(yearly_data["employee_contribution"]+yearly_data["employee_contribution"],2)
        if age==current_age+1:
            yearly_data["current_balance_without_employee_match"]=round(yearly_data["employee_contribution"]*(1+expected_annual_return-annual_investment_fee_rate),2)
            yearly_data["current_balance_with_employee_match"]=round(yearly_data["total_contribution"]*(1+expected_annual_return-annual_investment_fee_rate),2)
        else:
            yearly_data["current_balance_without_employee_match"]=report_data[-1]["current_balance_without_employee_match"]+round(yearly_data["employee_contribution"]*(1+expected_annual_return-annual_investment_fee_rate),2)
            yearly_data["current_balance_with_employee_match"]=report_data[-1]["current_balance_with_employee_match"]+round(yearly_data["total_contribution"]*(1+expected_annual_return-annual_investment_fee_rate),2)            
        yearly_data["current_withdraw_amount"]=round(yearly_data["current_balance_with_employee_match"]*(1+(0 if age>59 else -0.1)),2)
...
def trad_ira_calculator( 
    ...
    for age in range(current_age+1, retirement_age+1):
        ...
        age_data["taxable_balance"] = (taxable_balance + age_data["taxable_deposit"]) * (1 + expected_annual_return / 100)
